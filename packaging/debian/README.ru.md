...
Панель создаёт, клонирует и удаляет инстансы, формирует конфигурации и ключи, запускает службы, показывает журналы и диагностику, формирует Spec URI и QR, а также устанавливает обновления. Перед изменением и удалением создаётся root-only backup. Без `--start` placeholder-конфиг не запускается автоматически.
