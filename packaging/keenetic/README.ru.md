...
- смена логина и пароля с завершением всех сессий;
- окончательное удаление старого клиента после успешного переключения и резервной копии.
- полная форма DNS, SOCKS, liveness, lifecycle, traffic и transport-параметров;
- локальные резервные копии с SHA-256 и восстановление с автоматическим возвратом при ошибке.
...
После успешной проверки удалить старую службу, конфигурацию и бинарник можно кнопкой в Web UI или командой:

```sh
/opt/lib/olcrtc-keenetic/migration.sh purge-legacy
```

Команда выполняется только при живом native-клиенте и наличии сохранённой отключённой службы. Копия помещается в `/opt/var/lib/olcrtc-keenetic/legacy-backups`.
